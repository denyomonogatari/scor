	...
	<div class="collapse navbar-collapse" id="navbarText">
		<ul class="navbar-nav mr-auto">
			...
			<li class="nav-item"><a class="nav-link"
				href="SemesterPage?semesterId=${semesterId}">Courses</a></li>
		...
		<div class="row">
			<div class="col">
				...
				<c:forEach items="${assessments}" var="assessment"
					varStatus="status1">
					<form
						action="../actions/EditAssessment?semesterId=${semesterId}&courseId=${courseId}&assessmentId=${status1.index}"
						method="post" class="form-inline">
						<div class="form-group">
							<h2>
								<input type="text" class="form-control-plaintext"
									name="assessmentType" value="${assessment.type}">
							</h2>
						</div>
						<div class="form-group">
							<div class="input-group">
								<span><fmt:formatNumber type="number"
										maxFractionDigits="2" value="${assessment.score}" />% out of</span>
							</div>
							...
							<input type="text" class="form-control form-control-plaintext"
								name="assessmentWorth" value="${assessment.worth}" size="5">%
						</div>
						<div class="form-group">
							<input type="submit" class="btn btn-link" value="Save"> <a
								href="../actions/DeleteAssessment?semesterId=${semesterId}&courseId=${courseId}&assessmentId=${status1.index}">Delete</a>
						</div>
					</form>
					...
					<!-- start of form -->
					<c:forEach items="${assessment.assignments}" var="assignment"
						varStatus="status2">
						<c:set var="queryString" scope="page"
							value="?semesterId=${semesterId}&courseId=${courseId}&assessmentId=${status1.index}&assignmentId=${status2.index}" />
						<form action="../actions/EditAssignment${queryString}"
							method="post">
							<div class="form-row table">
								...
								<div class="col-2">
									<input type="text" class="form-control" placeholder="Score"
										name="assignmentScore" value="${assignment.score}">
								</div>
								<div class="col-2">
									<input type="text" class="form-control" placeholder="Worth"
										name="assignmentWorth" value="${assignment.total}">
								</div>
								<div class="col-1">
									<select class="custom-select" id="isDone" name="isDone">
										<option value="true"
											<c:if test="${assignment.completed}">selected</c:if>>Yes</option>
										<option value="false"
											<c:if test="${not assignment.completed}">selected</c:if>>No</option>
									</select>
								</div>
								<div class="col-3">
									<!-- TODO Fix disability when Done? is yes -->
									<input type="datetime-local" class="form-control" id="dueDate"
										name="dueDate" value="${assignment.dueDateString}">
								</div>
								<div class="col-2 center">
									<input type="submit" class="btn btn-secondary" value="Save"
										required> <a class="btn btn-primary btn-danger"
										href="../actions/DeleteAssignment${queryString}">Delete</a>
								</div>
							</div>
						</form>
					</c:forEach>
					<c:set var="queryString1" scope="page"
						value="?semesterId=${semesterId}&courseId=${courseId}&assessmentId=${status1.index}" />
					<form action="../actions/AddAssignment${queryString1}"
						method="post">
						<div class="form-row table">
							<div class="col-2">
								<input type="text" class="form-control"
									placeholder="Assignment name" name="assignmentName" required>
							</div>
							<div class="col-2">
								<input type="text" class="form-control" placeholder="Score"
